...
class RLNetworkEnv:
    ...
    def __init__(self, flow_profile, flow_path, reprofiling_delay, simulation_time=1000, shaping_mode="per_flow",
                 ...
                 packet_size=1, tor=0.003, pause_interval=1, high_reward=1, low_reward=0.1,
                 penalty=-10):
        self.simulator = NetworkSimulator(flow_profile, flow_path, reprofiling_delay, simulation_time=simulation_time,
                                          shaping_mode=shaping_mode, arrival_pattern_type=arrival_pattern_type,
    ...
    def reward_function(self, end_to_end_delay, flow_idx):
        ...
        reward = 0
        if end_to_end_delay <= self.simulator.latency_target[flow_idx]:
    ...
    def step(self, action):
        ...
        for flow_idx, (old_count, new_count) in enumerate(zip(packet_count_old, self.simulator.packet_count)):
            flow_end_to_end = []
            for packet_count in range(old_count, new_count):
                flow_end_to_end.append(
                    self.simulator.departure_time[flow_idx][packet_count][-1] - self.simulator.arrival_time[flow_idx][
                        packet_count])
        ...
        for flow_idx, end in enumerate(end_to_end):
            ...
            for e in end:
                ...
                if e > self.simulator.latency_target[flow_idx]:
                    exceed_target = True
            if self.simulator.packet_count[flow_idx] < len(self.simulator.arrival_time[flow_idx]):
                terminate = False
