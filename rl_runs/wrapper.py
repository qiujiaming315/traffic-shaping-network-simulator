...
def make_env(args, log_dir, continuous=False, same_seed=True):
    ...
    if same_seed:
        ...
        test_envs = SubprocVectorEnv(
            [lambda: make_one_env(args, continuous=continuous, log_path=log_paths[s],
    ...
    if same_seed:
        eval_envs = [DummyVectorEnv(
            [lambda: make_one_env(args, continuous=continuous, log_path=log_paths[s], enable_taad=enable_taad,
                                  save_delay_stats=True, save_action=True, save_reward=True, seed=args.seed)])
            for s in range(args.test_num)]
    else:
        eval_envs = [DummyVectorEnv(
            [lambda: make_one_env(args, continuous=continuous, log_path=log_paths[s], enable_taad=enable_taad,
                                  save_delay_stats=True, save_action=True, save_reward=True,
                                  seed=args.seed + args.training_num + s + 1)]) for s in range(args.test_num)]
    return eval_envs


def make_one_env(args, continuous=False, log_path="", enable_taad=False, save_delay_stats=False, save_action=False,
                 save_reward=False, seed=None):
    # Load the input data.
    ...
    env = RLNetworkEnv(flow_profile, flow_route, shaping_delay, simulation_time=args.simulation_time,
                       ...
                       arrival_pattern=None, keep_per_hop_departure=False, fill_tb_inter_arrival=enable_taad,
                       scaling_factor=1.0, packet_size=args.packet_size, propagation_delay=0,
                       ...
                       log_path=log_path, save_delay_stats=save_delay_stats,
                       save_action=save_action, save_reward=save_reward, seed=seed)
    return env
